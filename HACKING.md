...
Meson is the only supported build system here. Create a build dir with `meson setup build -Denable_debug_logging=true -Denable_xattr=true` (adjust options from `meson_options.txt` as needed), rebuild via `meson compile -C build`, and install or run with `meson install -C build` or `./build/wget --version`. `meson test -C build` now includes several suites (e.g., `signals_libev_dispatch`, `evhelpers_wait_and_sleep`, `http_response_stat_helpers`, `sitemap_parsing`). Lean on `meson test -C build` to run all available tests, or `meson test -C build <test_name>` to run specific tests. When you add options, wire them through Meson rather than reintroducing configure scripts. Remember that checklist hard requirements (c-ares, libev, optional TLS backends, worker pools) must be detectable/configurable from Meson, so extend `meson_options.txt` and `config.h` when new capabilities require toggles.
