...
Run `./bootstrap` whenever gnulib modules or build scripts change; it fetches dependencies and regenerates autotools glue. Configure with `./configure --enable-assert --with-ssl=openssl` to turn on assertions and build against the supported TLS backend. Use `make -j$(nproc)` for fast builds, `make check` to exercise both suites, and `src/wget --version` to verify the binary. `make syntax-check` (gnulib) enforces stylistic rules before submission.
...
Prefer OpenSSL (the only TLS backend) and document any non-default library paths (e.g., ``PKG_CONFIG_PATH=/opt/openssl/lib/pkgconfig``) in your patch description. When adding network-facing code, run under Valgrind/ASan and mention the results so reviewers can focus on logic rather than memory hygiene.
