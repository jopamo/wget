...
void windows_main(char** exec_name) {
  char* p;
...
static void ws_hangup(const char* reason) {
...
static int fake_fork_child(void) {
  ...
  if (!opt.lfilename && (!opt.quiet || opt.server_response)) {
    ...
    if (new_log_fp) {
      ...
      snprintf(info->lfilename, sizeof(info->lfilename), "%s", opt.lfilename);
      fclose(new_log_fp);
    }
  }
...
static void fake_fork(void) {
  ...
  xzero(si);
  si.cb = sizeof(si);
  rv = CreateProcess(exe, GetCommandLine(), NULL, NULL, TRUE, CREATE_SUSPENDED | DETACHED_PROCESS, NULL, NULL, &si, &pi);
  if (!rv)
    goto cleanup;
  ...
  section = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(struct fake_fork_info), name);
  le = GetLastError();
  xfree(name);
...
static BOOL WINAPI ws_handler(DWORD dwEvent) {
  switch (dwEvent) {
#ifdef CTRLC_BACKGND
    case CTRL_C_EVENT:
      ws_hangup("CTRL+C");
...
/* Updates the console title with the URL of the current file being
   transferred.  */
void ws_changetitle(const char* url) {
  xfree(title_buf);
  xfree(curr_url);
...
/* Updates the console title with the percentage of the current file
   transferred.  */
void ws_percenttitle(double percentage_float) {
  int percentage;
...
static void set_sleep_mode(void) {
  ...
  set_exec_state = (func_t)GetProcAddress(GetModuleHandle("KERNEL32.DLL"), "SetThreadExecutionState");
...
bool run_with_timeout(double seconds, void (*fun)(void*), void* arg) {
  HANDLE thread_hnd;
...
void set_windows_fd_as_blocking_socket(int fd) {
  /* 04/2011
  ...
  */
  ...
  if (ret) {
    fprintf(stderr, _("ioctl() failed.  The socket could not be set as blocking.\n"));
    DEBUGP(("Winsock error: %d\n", WSAGetLastError()));
    abort();
  }
  return;
