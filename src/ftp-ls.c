...
static int symperms(const char* s) {
  ...
  for (i = 0; i < 3; i++, s += 3) {
    perms <<= 3;
    perms += (((s[0] == 'r') << 2) + ((s[1] == 'w') << 1) + (s[2] == 'x' || s[2] == 's'));
  }
  return perms;
}
...
static struct fileinfo* ftp_parse_unix_ls(FILE* fp, int ignore_perms) {
    ...
    while (ptok = tok, (tok = strtok(NULL, " ")) != NULL) {
      ...
      else if (next == 2) /* The file name */
      {
        ...
        if (fnlen) {
          ...
          else if (cur.type == FT_PLAINFILE && (cur.perms & 0111) && cur.name[fnlen - 1] == '*') {
            cur.name[fnlen - 1] = '\0';
...
static struct fileinfo* ftp_parse_winnt_ls(FILE* fp) {
  int len;
  int year, month, day; /* for time analysis */
  int hour, min;
  size_t bufsize = 0;
...
static struct fileinfo* ftp_parse_vms_ls(FILE* fp) {
  int dt, i, j, len;
  int perms;
  size_t bufsize = 0;
  time_t timenow;
...
struct fileinfo* ftp_parse_ls(const char* file, const enum stype system_type) {
  FILE* fp;
  struct fileinfo* fi;
...
struct fileinfo* ftp_parse_ls_fp(FILE* fp, const enum stype system_type) {
  switch (system_type) {
    case ST_UNIX:
