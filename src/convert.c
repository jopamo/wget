...
static const char* replace_attr_refresh_hack(const char*, int, FILE*, const char*, int);
...
static void downloaded_files_free(void) {
  if (downloaded_files_hash) {
    ...
    for (hash_table_iterate(downloaded_files_hash, &iter); hash_table_iter_next(&iter);)
      xfree(iter.key);
