...
/* Look for regexp "( *[0-9]+ *byte" (literal parenthesis) anywhere in
   ...
   otherwise.  */
static wgint ftp_expected_bytes(const char* s) {
  wgint res;
...
static uerr_t get_ftp_greeting(int csock, ccon* con) {
  uerr_t err = 0;
...
#define BIN_TYPE_FILE ((!(cmd & DO_LIST)) && BIN_TYPE_TRANSFER && (opt.ftp_stmlf == 0))
...
#define BIN_TYPE_FILE true
      ...
      else if (opt.noclobber || opt.always_rest || opt.timestamping || opt.dirstruct || opt.output_document || count > 0) {
    ...
    if (res == -2 || (warc_tmp != NULL && res == -3)) {
      ...
      logprintf(LOG_NOTQUIET, _("%s (%s) - Data connection: %s; "), tms, tmrate, fd_errstr(dtsock));
      if (opt.server_response)
        logputs(LOG_ALWAYS, "\n");
    }
    fd_close(dtsock);
...
static uerr_t ftp_loop_internal(struct url* u, struct url* original_url, struct fileinfo* f, ccon* con, char** local_file, bool force_full_retrieve) {
  ...
  if (opt.noclobber && !opt.output_document && file_exists_p(con->target, NULL) && !((con->cmd & DO_LIST) && !(con->cmd & DO_RETR))) {
  ...
  do {
    ...
    else if (opt.always_rest && stat(locf, &st) == 0 && S_ISREG(st.st_mode))
    ...
    if (!opt.spider) {
      ...
      logprintf(LOG_VERBOSE, write_to_stdout ? _("%s (%s) - written to stdout %s[%s]\n\n") : _("%s (%s) - %s saved [%s]\n\n"), tms, tmrate, write_to_stdout ? "" : quote(locf),
    ...
    if (warc_enabled && (con->cmd & DO_RETR)) {
      ...
      warc_res = warc_write_resource_record(NULL, u->url, NULL, NULL, warc_ip, NULL, warc_tmp, -1);
...
static uerr_t ftp_retrieve_dirs(struct url*, struct url*, struct fileinfo*, ccon*);
...
#ifdef HAVE_SYMLINK
          if (!f->linkto)
            logputs(LOG_NOTQUIET, _("Invalid name of the symlink, skipping.\n"));
          else {
            struct stat st;
...
Already have correct symlink %s -> %s\n\n"),
          ...
          logprintf(LOG_NOTQUIET, _("Symlinks not supported, skipping symlink %s.\n"), quote(con->target));
#endif /* not HAVE_SYMLINK */
        }
        else /* opt.retr_symlinks */
        {
          if (dlthis) {
            err = ftp_loop_internal(u, original_url, f, con, NULL, force_full_retrieve);
          }
        } /* opt.retr_symlinks */
        break;
      case FT_DIRECTORY:
    ...
    if (dlthis && (actual_target != NULL) && (f->type == FT_PLAINFILE) && opt.preserve_perm) {
      if (f->perms) {
    ...
    if (actual_target != NULL) {
      if (opt.useservertimestamps && !(f->type == FT_SYMLINK && !opt.retr_symlinks) && f->tstamp != -1 && dlthis && file_exists_p(con->target, NULL)) {
  ...
  if (opt.recursive && !(opt.reclevel != INFINITE_RECURSION && depth >= opt.reclevel))
...
static bool has_insecure_name_p(const char* s) {
  if (*s == '/')
    return true;
...
static uerr_t ftp_retrieve_glob(struct url* u, struct url* original_url, ccon* con, int action) {
   ...
   * harmless.
   */
  int (*cmp)(const char*, const char*) = opt.ignore_case ? strcasecmp : (int (*)())strcmp;
#else  /* def __VMS */
  int (*cmp)(const char*, const char*) = opt.ignore_case ? strcasecmp : strcmp;
#endif /* def __VMS [else] */
  ...
  while (f) {
    ...
    if ((opt.accepts || opt.rejects) && f->type != FT_DIRECTORY && !acceptable(f->name)) {
    ...
    if (opt.acceptregex || opt.rejectregex) {
      ...
      if ((unsigned)snprintf(buf, sizeof(buf), "%s%s%s", u->url, f->name, f->type == FT_DIRECTORY ? "/" : "") >= sizeof(buf)) {
  ...
  if (!*u->file && !recursive) {
    ...
    if (res == RETROK) {
      if (opt.htmlify && !opt.spider) {
        ...
        char* filename = (opt.output_document ? xstrdup(opt.output_document) : (con.target ? xstrdup(con.target) : url_file_name(url_file, NULL)));
        ...
        if (res == FTPOK && opt.verbose) {
          if (!opt.output_document) {
            ...
            if (stat(filename, &st) == 0)
              ...
              sz = -1;
            logprintf(LOG_NOTQUIET, _("Wrote HTML-ized index to %s [%s].\n"), quote(filename), number_to_static_string(sz));
          }
          else
            logprintf(LOG_NOTQUIET, _("Wrote HTML-ized index to %s.\n"), quote(filename));
        }
        xfree(filename);
      }
      freefileinfo(f);
