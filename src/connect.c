...
/* Fill SA as per the data in IP and PORT
   SA should point to struct sockaddr_storage if ENABLE_IPV6 is defined,
   to struct sockaddr_in otherwise */
...
/* Get the data of SA, specifically the IP address and the port
   If you're not interested in one or the other information, pass NULL
   as the pointer */
...
/* Return the size of the sockaddr structure depending on its family */
...
/* Resolve the bind address specified via --bind-address and store it
   to SA
   The resolved value is stored in a static variable and reused after
...
static bool resolve_bind_address(struct sockaddr* sa) {
  ...
  /* Make sure this is called only once
     opt.bind_address doesn't change during a Wget run */
  ...
  /* Pick the first address in the list and use it as bind address
     Perhaps we should try multiple addresses in succession, but this
     is typically unnecessary in practice */
...
/* Like connect, but specifies a timeout
   If connecting takes longer than TIMEOUT seconds, -1 is returned and
...
int connect_to_ip(const ip_address* ip, int port, const char* print) {
  ...
  if (opt.ipv6_only) {
    ...
    int err = setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, &on, (socklen_t)sizeof(on));
    IF_DEBUG
    if (err < 0)
...
/* Connect via TCP to a remote host on the specified port
   HOST is resolved as an Internet host name
   If HOST resolves to more than one IP address, they are tried in the
   order returned by DNS until connecting to one of them succeeds */
...
retry:
  ...
  for (i = start; i < end; i++) {
    ...
    /* The attempt to connect has failed
       Continue with the loop and try next address */
  ...
  if (address_list_connected_p(al)) {
    /* We connected to AL before, but cannot do so now
       That might indicate that our DNS cache entry for HOST has
       expired */
    address_list_release(al);
...
/* Create a socket, bind it to local interface BIND_ADDRESS on port
   *PORT, set up a listen backlog, and return the resulting socket, or
   -1 in case of error
   BIND_ADDRESS is the address of the interface to bind to
   If it is NULL, the socket is bound to the default address
   PORT should point to the port number that will be used for the binding
   If that number is 0, the system will choose a suitable port, and the
...
int bind_local(const ip_address* bind_address, int* port) {
  ...
  socklen_t setopt_size = (socklen_t)sizeof(setopt_val);
...
/* Like a call to accept(), but with the added check for timeout
   ...
   The caller is blocked until a connection is established
   If no connection is established for opt.connect_timeout seconds,
   the function exits with an error status */
...
/* Get the IP address associated with the connection on FD and store
   ...
   Else if ENDPOINT is ENDPOINT_PEER, it returns the address of the
   remote (peer's) side of the socket */
...
void fd_register_transport(int fd, struct transport_implementation* imp, void* ctx) {
  ...
  /* The file descriptor must be non-negative to be registered
     Negative values are ignored by fd_close(), and -1 cannot be used
     as hash key */
  assert(fd >= 0);
...
/* Return context of the transport registered with fd_register_transport
   This assumes fd_register_transport was previously called on FD */
...
/* When fd_read/fd_write are called multiple times in a loop, they
   should remember the INFO pointer instead of fetching it every time
