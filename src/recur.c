 ...
 * src/recur.c
 *
 * Breadth-first recursive retrieval, URL queue management, robots rules,
 * and reject logging
 */
...
/* FIFO queue of URLs scheduled for retrieval */

struct queue_element {
  const char* url;            /* URL to download */
  const char* referer;        /* referring document URL */
  int depth;                  /* recursion depth */
  bool html_allowed;          /* allow treating document as HTML */
  struct iri* iri;            /* per-URL IRI context */
  bool css_allowed;           /* allow treating document as CSS */
...
/* Enqueue URL for later retrieval, preserving insertion order */
...
/* Dequeue next URL from the queue
...
/* Blacklist helpers track URLs that must not be enqueued again */
 ...
 *
 * Uses a FIFO queue, a blacklist for URLs already scheduled, and
 * robots rules and depth/acceptance checks to decide which children
 * to enqueue
 */
...
#undef COPYSTR
  ...
  for (;;) {
    ...
    if (!url_dequeue(queue, &i, (const char**)&url, (const char**)&referer, &depth, &html_allowed, &css_allowed))
      break;

    /* If URL already downloaded, reuse local file and only parse children */
    if (dl_url_file_map && hash_table_contains(dl_url_file_map, url)) {
      ...
      if (!url_parsed) {
        ...
        /* css_allowed can override content type, many servers mislabel CSS */
        ...
        if (redirected) {
          /* follow redirects only if children are allowed */
          if (descend) {
            reject_reason r = descend_redirect(redirected, url_parsed, depth, start_url_parsed, blacklist, i);
            if (r == WG_RR_SUCCESS) {
              /* blacklist original pre-redirect URL so we don't requeue it */
    ...
    /* depth limiting with optional -p page requisites overflow */
    if (descend && depth >= opt.reclevel && opt.reclevel != INFINITE_RECURSION) {
      if (opt.page_requisites && (depth == opt.reclevel || depth == opt.reclevel + 1)) {
        /* allow one extra level for inline requisites (and one more for frames) */
    ...
    /* parse HTML/CSS and enqueue children */
      ...
      if (children) {
        ...
        for (; child; child = child->next) {
          ...
          if (r == WG_RR_SUCCESS) {
            ...
            /* we never want to enqueue the same URL twice */
    ...
    /* delete local file if we only needed it for parsing or it fails filters */
  ...
  /* drain and free any leftover queued entries on premature exit */
  {
    char *d1, *d2;
    int d3;
    bool d4, d5;
    ...
    while (url_dequeue(queue, &d6, (const char**)&d1, (const char**)&d2, &d3, &d4, &d5)) {
      iri_free(d6);
      xfree(d1);
      xfree(d2);
    }
...
/* Decide whether a child URL should be enqueued, based on:
 * - scheme and https-only
 * - relative-only
 * - domain/no-parent rules
 * - include/exclude lists and regex filters
 * - depth, suffix, and spanhost
 * - robots.txt policy
 */
...
static reject_reason download_child(const struct urlpos* upos, struct url* parent, int depth, struct url* start_url_parsed, struct hash_table* blacklist, struct iri* iri) {
  ...
  /* HTTP-like schemes are treated similarly; FTP/FTPS are optional */
  ...
  /* enforce no-parent unless scheme/host changes or URL is inline -p requisite */
  if (opt.no_parent && schemes_are_similar_p(u->scheme, start_url_parsed->scheme) && strcasecmp(u->host, start_url_parsed->host) == 0 &&
  ...
  /* directory include/exclude rules */
  ...
  /* suffix/filename rules, with exceptions for HTML that may lead to
   * further downloads or -p requisites
   */
  ...
  if (schemes_are_similar_p(u->scheme, parent->scheme) && !opt.spanhost && strcasecmp(parent->host, u->host) != 0) {
  ...
  if (opt.use_robots && u_scheme_like_http) {
    ...
    if (!specs) {
      ...
      if (res_retrieve_file(url, &rfile, iri)) {
        ...
        /* remove robots.txt if we only needed it for policy checks */
        ...
        /* dummy specs so we do not keep trying to fetch robots.txt */
        specs = res_parse("", 0);
      }
...
/* Evaluate whether children of a redirected URL should be followed
 * Wrapper around download_child that special-cases list/regex rejections
 */
...
/* Write reject log header once per log file */

static void write_reject_log_header(FILE* f) {
  if (!f)
    return;

  /* Update this header if columns change */
  fprintf(f,
          "REASON\t"
...
/* Helper for writing one URL in reject log row */
...
/* Log a single rejection with reason, rejected URL, and parent URL
 * Uses tab-separated fields for easy post-processing
 */
