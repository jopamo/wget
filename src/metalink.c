...
uerr_t retrieve_from_metalink(const metalink_t* metalink) {
  ...
  if (opt.output_document) {
    ...
    logputs(LOG_NOTQUIET, _("-O not supported for metalink download. Ignoring.\n"));
  }
  ...
  for (mfile_ptr = metalink->files; *mfile_ptr; mfile_ptr++) {
    ...
    if (!safename || strcmp(planname, safename)) {
      logprintf(LOG_NOTQUIET, _("[--trust-server-names %s, --directory-prefix=%s]\n"), (opt.trustservernames ? "true" : "false"), quote(opt.dir_prefix ? opt.dir_prefix : ""));
      logprintf(LOG_NOTQUIET, _("Planned metalink file: %s\n"), quote(planname ? planname : ""));
      logprintf(LOG_NOTQUIET, _("Secured metalink file: %s\n"), quote(safename ? safename : ""));
      if (!safename) {
        logprintf(LOG_NOTQUIET, _("Rejecting metalink file. Unsafe name.\n"));
        xfree(planname);
        xfree(trsrname);
        xfree(filename);
        continue;
    ...
    if (opt.metalink_index >= 0) {
      ...
      if (mfile->metaurls && mfile->metaurls[0])
        for (murl_ptr = mfile->metaurls; *murl_ptr; murl_ptr++) {
          ...
          logprintf(LOG_NOTQUIET, _("Processing metaurl %s...\n"), quote(murl->url));
          ...
          if (!metalink_check_safe_path(metafile)) {
            logprintf(LOG_NOTQUIET, _("Rejecting metaurl file %s. Unsafe name.\n"), quote(metafile));
            xfree(metafile);
            if (_metalink_index > 0)
              break;
            continue;
          ...
          x_retr_err = fetch_metalink_file(murl->url, false, false, metafile, &metadest);

          /* On failure, try the next metalink metaurl.  */
          if (x_retr_err != RETROK) {
            logprintf(LOG_VERBOSE, _("Failed to download %s. Skipping metaurl.\n"), quote(metadest ? metadest : metafile));
          ...
          /* On failure, try the next metalink metaurl.  */
          if (meta_err) {
            logprintf(LOG_NOTQUIET, _("Unable to parse metaurl file %s.\n"), quote(metadest));
          ...
          if (opt.preferred_location && opt.preferred_location[0]) {
            ...
            for (x_mfile_ptr = metaurl_xml->files; *x_mfile_ptr; x_mfile_ptr++) {
              ...
              stable_sort(x_mfile->resources, mres_count, sizeof(metalink_resource_t*), metalink_res_cmp);
            }
          }
    ...
    for (mres_ptr = mfile->resources; *mres_ptr && mfile->checksums && !skip_mfile; mres_ptr++) {
      ...
      if (!RES_TYPE_SUPPORTED(mres->type)) {
        logprintf(LOG_VERBOSE, _("Resource type %s not supported, ignoring...\n"), quote(mres->type));
        continue;
      }
      ...
      if (!url) {
        ...
        */
        ...
        DEBUGP(("Storing to %s\n", destname));
        {
          ...
          retr_err = retrieve_url(url, mres->url, NULL, NULL, NULL, NULL, opt.recursive, iri, false, &tctx);
      ...
      if (retr_err == RETROK) {
        size_ok = false;
        hash_ok = false;
        wget_metalink_verify_checksums(mfile, destname, &size_ok, &hash_ok);
        ...
        if (mfile->signature) {
          ...
          if (fd == -1) {
            logputs(LOG_NOTQUIET, _("Could not open downloaded file for signature "
                                    "verification.\n"));
          ...
          if (gpgerr != GPG_ERR_NO_ERROR) {
            logprintf(LOG_NOTQUIET, "GPGME data_new_from_fd: %s\n", gpgme_strerror(gpgerr));
            goto gpg_skip_verification;
          }
          ...
          if (gpgerr != GPG_ERR_NO_ERROR) {
            logprintf(LOG_NOTQUIET, "GPGME new: %s\n", gpgme_strerror(gpgerr));
            gpgme_data_release(gpgdata);
            goto gpg_skip_verification;
          }

          DEBUGP(("Verifying signature %s:\n%s\n", quote(msig->mediatype), msig->signature));
          ...
          if (gpgerr != GPG_ERR_NO_ERROR) {
            logprintf(LOG_NOTQUIET, "GPGME set_protocol: %s\n", gpgme_strerror(gpgerr));
            gpgme_release(gpgctx);
            gpgme_data_release(gpgdata);
            goto gpg_skip_verification;
          }
          ...
          /* Load the signature.  */
          gpgerr = gpgme_data_new_from_mem(&gpgsigdata, msig->signature, strlen(msig->signature), 0);
          if (gpgerr != GPG_ERR_NO_ERROR) {
            logprintf(LOG_NOTQUIET, _("GPGME data_new_from_mem: %s\n"), gpgme_strerror(gpgerr));
            gpgme_release(gpgctx);
            gpgme_data_release(gpgdata);
            goto gpg_skip_verification;
          }
          ...
          if (gpgerr != GPG_ERR_NO_ERROR) {
            logprintf(LOG_NOTQUIET, _("GPGME op_verify: %s\n"), gpgme_strerror(gpgerr));
          ...
          if (!gpgres) {
            logputs(LOG_NOTQUIET, _("GPGME op_verify_result: NULL\n"));
            gpgme_data_release(gpgsigdata);
            gpgme_release(gpgctx);
            gpgme_data_release(gpgdata);
            goto gpg_skip_verification;
          ...
          for (gpgsig = gpgres->signatures; gpgsig; gpgsig = gpgsig->next) {
            ...
            if (gpgsig->summary & (GPGME_SIGSUM_VALID | GPGME_SIGSUM_GREEN)) {
              logputs(LOG_VERBOSE, _("Signature validation succeeded.\n"));
              sig_status = 1;
              break;
            }

            if (gpgsig->summary & GPGME_SIGSUM_RED) {
              logputs(LOG_NOTQUIET, _("Invalid signature. Rejecting resource.\n"));
              sig_status = -1;
              break;
            }

            if (gpgsig->summary == 0 && (gpgsig->status & 0xFFFF) == GPG_ERR_NO_ERROR) {
              logputs(LOG_VERBOSE, _("Data matches signature, but signature "
                                     "is not trusted.\n"));
            }

            if ((gpgsig->status & 0xFFFF) != GPG_ERR_NO_ERROR) {
              logprintf(LOG_NOTQUIET, "GPGME: %s\n", gpgme_strerror(gpgsig->status & 0xFFFF));
            }
          }
          gpgme_data_release(gpgsigdata);
          gpgme_release(gpgctx);
    ...
    if (((retr_err != RETROK && !opt.always_rest) || opt.delete_after) && destname != NULL && file_exists_p(destname, NULL)) {
      badhash_or_remove(destname);
    }
    if (output_stream) {
      fclose(output_stream);
...
*/
uerr_t fetch_metalink_file(const char* url_str, bool resume, bool metalink_http, const char* filename, char** destname) {
  FILE* _output_stream = output_stream;
  bool _output_stream_regular = output_stream_regular;
  char* _output_document = opt.output_document;
  bool _metalink_http = opt.metalink_over_http;
  ...
  */
  ...
  DEBUGP(("Storing to %s\n", local_file));
  {
    ...
    retr_err = retrieve_url(url, url_str, NULL, NULL, NULL, NULL, opt.recursive, iri, false, &tctx);
    transfer_context_free(&tctx);
  }
...
int metalink_res_cmp(const void* v1, const void* v2) {
  const metalink_resource_t *res1 = *(metalink_resource_t**)v1, *res2 = *(metalink_resource_t**)v2;
  ...
  if (opt.preferred_location) {
    int cmp = 0;
    if (res1->location && !c_strcasecmp(opt.preferred_location, res1->location))
      cmp -= 1;
    if (res2->location && !c_strcasecmp(opt.preferred_location, res2->location))
      cmp += 1;
    return cmp;
  }
  return 0;
...
int metalink_meta_cmp(const void* v1, const void* v2) {
  const metalink_metaurl_t *meta1 = *(metalink_metaurl_t**)v1, *meta2 = *(metalink_metaurl_t**)v2;
...
/*
 ...
 See more: rfc5988
*/
bool find_key_value(const char* start, const char* end, const char* key, char** value) {
  const char* eq;
 ...
 */
const char* find_key_values(const char* start, const char* end, char** key, char** value) {
  ...
  while (val_end < end && *val_end != ';' && *val_end != ',' && !c_isspace(*val_end))
    val_end++;
...
const char* test_find_key_values(void) {
  ...
  static const struct {
    const char* key;
    const char* val;
  } test_array[] = {{"key1", "val1"}, {"key2", "val2"}, {"key3", "val3"}, {"key4", "val4"},   {"key5", "val5"},  {"key6", "val6"},
                    {"key7", "val7"}, {"key8", "val8"}, {"key9", "val9"}, {"key10", "val10"}, {"key12", "val12"}};
  const char* pos;
  char *key, *value;
  size_t i = 0;

  for (pos = header_data; (pos = find_key_values(pos, header_data + strlen(header_data), &key, &value)); pos++) {
    mu_assert("test_find_key_values: wrong result", !strcmp(test_array[i].val, value) && !strcmp(test_array[i].key, key));
    xfree(key);
    xfree(value);
    i++;
  }
...
const char* test_find_key_value(void) {
  ...
  static const struct {
    ...
    const char* val;
    bool result;
  } test_array[] = {{"key1", "val1", true}, {"key2", "val2", true}, {"key3", "val3", true}, {"key4", "val4", true}, {"key5", "val5", true}, {"key6", "val6", true},
                    {"key7", "val7", true}, {"key8", "val8", true}, {"key9", "val9", true}, {"key10", NULL, false}, {"ey1", NULL, false},   {"dey1", NULL, false}};
  size_t i;
  ...
  for (i = 0; i < countof(test_array); ++i) {
    ...
    result = find_key_value(header_data, header_data + strlen(header_data), test_array[i].key, &value);

    mu_assert("test_find_key_value: wrong result", result == test_array[i].result && ((!test_array[i].result && !value) || !strcmp(value, test_array[i].val)));
...
const char* test_has_key(void) {
  static const char* header_data =
      "key1=val2;token1;xyz; token2;xyz;token3 ;"
      "xyz; token4 ;xyz;   token5  ";
  struct {
    const char* token;
    bool result;
  } test_array[] = {{"key1=val2", true}, {"token1", true},  {"token2", true}, {"token3", true},  {"token4", true},
                    {"token5", true},    {"token6", false}, {"oken1", false}, {"poken1", false}, {"key1=val2", true}};
  size_t i;

  for (i = 0; i < countof(test_array); ++i)
    mu_assert("test_has_key: wrong result", has_key(header_data, header_data + strlen(header_data), test_array[i].token) == test_array[i].result);
