...
#define DECLARE_TAG_HANDLER(fun) static void fun(int, struct taginfo*, struct map_context*)
...
/* The list of known tags and functions used for handling them
   Most tags are simply harvested for URLs */
static struct known_tag {
  int tagid;
  const char* name;
  tag_handler_t handler;
} known_tags[] = {
    {TAG_A, "a", tag_find_urls},           {TAG_APPLET, "applet", tag_find_urls},   {TAG_AREA, "area", tag_find_urls},
    {TAG_BASE, "base", tag_handle_base},   {TAG_BGSOUND, "bgsound", tag_find_urls}, {TAG_BODY, "body", tag_find_urls},
    {TAG_EMBED, "embed", tag_find_urls},   {TAG_FIG, "fig", tag_find_urls},         {TAG_FORM, "form", tag_handle_form},
    {TAG_FRAME, "frame", tag_find_urls},   {TAG_IFRAME, "iframe", tag_find_urls},   {TAG_IMG, "img", tag_handle_img}, /* tag_find_urls() plus handling "srcset" */
    {TAG_INPUT, "input", tag_find_urls},   {TAG_LAYER, "layer", tag_find_urls},     {TAG_LINK, "link", tag_handle_link},
    {TAG_META, "meta", tag_handle_meta},   {TAG_OBJECT, "object", tag_find_urls},   {TAG_OVERLAY, "overlay", tag_find_urls},
    {TAG_SCRIPT, "script", tag_find_urls}, {TAG_TABLE, "table", tag_find_urls},     {TAG_TD, "td", tag_find_urls},
    {TAG_TH, "th", tag_find_urls},         {TAG_VIDEO, "video", tag_find_urls},     {TAG_AUDIO, "audio", tag_find_urls},
...
/* The link is "inline", i.e. needs to be retrieved for this document
   ...
   Inline links include inlined images, stylesheets, children frames, etc */
...
/* Will contain the (last) charset found in 'http-equiv=content-type'
   meta tags */
static char* meta_charset;

/* Initialize interesting_tags and interesting_attributes used by the HTML parser
   to know which tags and attributes we care about
   This is initialized only once for performance reasons */
static void init_interesting(void) {
  size_t i;

  interesting_tags = make_nocase_string_hash_table(countof(known_tags));

  /* First, add all the tags we know how to handle, mapped to their
  ...
  for (i = 0; i < countof(tag_url_attributes); i++)
    hash_table_put(interesting_attributes, tag_url_attributes[i].attr_name, "1");
}

/* Find the value of attribute named NAME in the taginfo TAG
   If the attribute is not present, return NULL
   If ATTRIND is non-NULL, the index of the attribute in TAG will be stored there */
static char* find_attr(struct taginfo* tag, const char* name, int* attrind) {
  int i;
...
#define ATTR_POS(tag, attrind, ctx) (tag->attrs[attrind].value_raw_beginning - (ctx)->text)
#define ATTR_SIZE(tag, attrind) (tag->attrs[attrind].value_raw_size)
...
struct urlpos* append_url(const char* link_uri, int position, int size, struct map_context* ctx) {
  ...
  if (!base) {
    ...
    if (!link_has_scheme) {
      /* Base URL is unavailable, and the link does not have a
         location attached to it -- we have to give up
         Since this can only happen when using `--force-html -i',
         print a warning */
      logprintf(LOG_NOTQUIET, _("%s: Cannot resolve incomplete link %s.\n"), ctx->document_file, link_uri);
      iri_free(iri);
      return NULL;
    }
  ...
  if (ctx->head == NULL)
    ...
    struct urlpos* it = ctx->head;
    struct urlpos* prev = NULL;
...
static void check_style_attr(struct taginfo* tag, struct map_context* ctx) {
  int attrind;
  int raw_start;
  int raw_len;
  ...
  /* raw pos and raw size include the quotes, skip them when they are present */
  raw_start = ATTR_POS(tag, attrind, ctx);
  raw_len = ATTR_SIZE(tag, attrind);

  if (ctx->text[raw_start] == '\'' || ctx->text[raw_start] == '"') {
    ++raw_start;
    raw_len -= 2;
  }
...
static void tag_find_urls(int tagid, struct taginfo* tag, struct map_context* ctx) {
  ...
  int attrind;
  int first = -1;
  const size_t attributes_count = countof(tag_url_attributes);
  ...
  for (attrind = 0; attrind < tag->nattrs; attrind++) {
    /* Find whether TAG/ATTRIND is a combination that contains a URL */
    ...
    for (i = (size_t)first; i < attributes_count && tag_url_attributes[i].tagid == tagid; i++) {
      if (strcasecmp(tag->attrs[attrind].name, tag_url_attributes[i].attr_name) == 0) {
        struct urlpos* up = append_url(link, ATTR_POS(tag, attrind, ctx), ATTR_SIZE(tag, attrind), ctx);
        if (up) {
...
static void tag_handle_base(int tagid WGET_ATTR_UNUSED, struct taginfo* tag, struct map_context* ctx) {
  ...
  base_urlpos = append_url(newbase, ATTR_POS(tag, attrind, ctx), ATTR_SIZE(tag, attrind), ctx);
  if (!base_urlpos)
    return;
...
static void tag_handle_form(int tagid WGET_ATTR_UNUSED, struct taginfo* tag, struct map_context* ctx) {
  ...
  if (action) {
    struct urlpos* up = append_url(action, ATTR_POS(tag, attrind, ctx), ATTR_SIZE(tag, attrind), ctx);
    if (up)
...
/* Handle the LINK tag
   It requires special handling because how its links will be followed
   in -p mode depends on the REL attribute */
static void tag_handle_link(int tagid WGET_ATTR_UNUSED, struct taginfo* tag, struct map_context* ctx) {
  ...
  if (href) {
    struct urlpos* up = append_url(href, ATTR_POS(tag, attrind, ctx), ATTR_SIZE(tag, attrind), ctx);
    if (up) {
      char* rel = find_attr(tag, "rel", NULL);
      if (rel) {
        if (c_strcasecmp(rel, "stylesheet") == 0 || c_strcasecmp(rel, "alternate stylesheet") == 0) {
          up->link_inline_p = 1;
          up->link_expect_css = 1;
        }
        else if (c_strcasecmp(rel, "shortcut icon") == 0 || c_strcasecmp(rel, "icon") == 0) {
          up->link_inline_p = 1;
        }
        else if (c_strcasecmp(rel, "manifest") == 0) {
          up->link_inline_p = 1;
        }
        else {
...
/* Handle the META tag
   This requires special handling because of the refresh feature and robot exclusion */
static void tag_handle_meta(int tagid WGET_ATTR_UNUSED, struct taginfo* tag, struct map_context* ctx) {
  ...
  if (http_equiv && c_strcasecmp(http_equiv, "refresh") == 0) {
    ...
    entry = append_url(p, ATTR_POS(tag, attrind, ctx), ATTR_SIZE(tag, attrind), ctx);
    if (entry) {
  ...
  else if (http_equiv && c_strcasecmp(http_equiv, "content-type") == 0) {
    /* Handle stuff like:
  ...
  else if (name && c_strcasecmp(name, "robots") == 0) {
    /* Handle stuff like:
...
/* Handle the IMG tag
   This requires special handling for the srcset attr, while the traditional
   src/lowsrc/href attributes can be handled generically */
static void tag_handle_img(int tagid, struct taginfo* tag, struct map_context* ctx) {
  int attrind;
  char* srcset;
  ...
  if (srcset) {
    ...
    while (offset < size) {
      ...
      /* Skip over initial whitespace and commas.  Note there is no \v
         in HTML5 whitespace */
      url_start = offset + (int)strspn(srcset + offset, " \f\n\r\t,");
      ...
      url_end = url_start + (int)strcspn(srcset + url_start, " \f\n\r\t");
      ...
      if (url_end > url_start) {
        char* url_text = strdupdelim(srcset + url_start, srcset + url_end);
        struct urlpos* up = append_url(url_text, base_ind + url_start, url_end - url_start, ctx);
      ...
      /* If the URL wasn't terminated by a comma there may also be a descriptor
         which we just skip */
      if (has_descriptor) {
...
/* Dispatch the tag handler appropriate for the tag we're mapping over
   See known_tags[] for definition of tag handlers */
static void collect_tags_mapper(struct taginfo* tag, void* arg) {
  ...
  /* Find the tag in our table of tags.  This must not fail because
     ...
     For now we pass NULL as allowed_tags to map_html_tags so that we
     can check every tag for style attributes while still using
     interesting_tags here to decide which tags get URL handlers */
  ...
  if (tag->end_tag_p && c_strcasecmp(tag->name, "style") == 0 && tag->contents_begin && tag->contents_end && tag->contents_begin <= tag->contents_end) {
    /* parse contents */
    get_urls_css(ctx, tag->contents_begin - ctx->text, tag->contents_end - tag->contents_begin);
  }
}

/* Analyze HTML tags in FILE and construct a list of URLs referenced from it
   It merges relative links in FILE with URL
   It is aware of <base href=...> and does the right thing */
...
struct urlpos* get_urls_html(const char* file, const char* url, bool* meta_disallow_follow, struct iri* iri) {
  struct urlpos* urls;
  struct file_memory* fm = wget_read_file(file);
...
struct urlpos* get_urls_file(const char* file, bool* read_again) {
  ...
  while (text < text_end) {
    ...
    /* The URL is in the [line_beg, line_end) region
       ...
       can't use alloca because we're in a loop */
