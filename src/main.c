    ...
    if (filename) {
      ...
      if (!hsts_store)
        logprintf(LOG_NOTQUIET,
                  "ERROR: could not open HSTS store at '%s'. "
...
static struct cmdline_option option_data[] = {
    ...
    IF_SSL("crl-file", 0, OPT_VALUE, "crlfile", -1){"cut-dirs", 0, OPT_VALUE, "cutdirs", -1},
    ...
    IF_SSL("https-only", 0, OPT_BOOLEAN, "httpsonly", -1){"ignore-case", 0, OPT_BOOLEAN, "ignorecase", -1},
    ...
    IF_SSL("secure-protocol", 0, OPT_VALUE, "secureprotocol", -1){"server-response", 'S', OPT_BOOLEAN, "serverresponse", -1},
...
HTTPS (SSL/TLS) options:\n"),
       ...
       --egd-file=FILE             file naming the EGD socket with random data\n"),
                               ...
                               "\n",
                               N_("\
       --ciphers=STR           Set the OpenSSL cipher list string directly.\n\
...
static void run_use_askpass(char* question, char** answer) {
  char tmp[1024];
  pid_t pid;
  int status;
  int com[2];
  ...
  if (status) {
    fprintf(stderr, _("Error initializing spawn file actions for use-askpass: %d\n"), status);
    exit(WGET_EXIT_GENERIC_ERROR);
  }

  status = posix_spawn_file_actions_adddup2(&fa, com[1], STDOUT_FILENO);
  if (status) {
    fprintf(stderr, _("Error setting spawn file actions for use-askpass: %d\n"), status);
    exit(WGET_EXIT_GENERIC_ERROR);
  }
  ...
  if (bytes <= 0) {
    fprintf(stderr, _("Error reading response from command \"%s %s\": %s\n"), opt.use_askpass, question, strerror(errno));
    exit(WGET_EXIT_GENERIC_ERROR);
  }
...
static void use_askpass(struct url* u) {
  static char question[1024];

  if (u->user == NULL || u->user[0] == '\0') {
    snprintf(question, sizeof(question), _("Username for '%s%s': "), scheme_leading_string(u->scheme), u->host);
    /* Prompt for username */
  ...
  if (u->passwd == NULL || u->passwd[0] == '\0') {
    snprintf(question, sizeof(question), _("Password for '%s%s@%s': "), scheme_leading_string(u->scheme), u->user, u->host);
    /* Prompt for password */
...
/* Function that prints the line argument while limiting it
   ...
   lines.*/
static int format_and_print_line(const char* prefix, const char* line, int line_length) {
  int remaining_chars;
  char *line_dup, *token;
...
#ifdef ENABLE_NLS
  if (format_and_print_line(locale_title, LOCALEDIR, MAX_CHARS_PER_LINE) < 0)
    exit(WGET_EXIT_IO_FAIL);
#endif /* def ENABLE_NLS */

  if (compilation_string != NULL)
    if (format_and_print_line(compile_title, compilation_string, MAX_CHARS_PER_LINE) < 0)
      exit(WGET_EXIT_IO_FAIL);

  if (link_string != NULL)
    if (format_and_print_line(link_title, link_string, MAX_CHARS_PER_LINE) < 0)
      exit(WGET_EXIT_IO_FAIL);
...
int main(int argc, char** argv) {
  ...
  *p = '\0';
  ...
  if (opt.noclobber && (opt.convert_links || opt.convert_file_only)) {
    fprintf(stderr, opt.convert_links ? _("Both --no-clobber and --convert-links were specified,"
...
Can't timestamp and not clobber old files at the same time.\n"));
  ...
  if (opt.output_document) {
    if ((opt.convert_links || opt.convert_file_only) && (nurls > 1 || opt.page_requisites || opt.recursive)) {
      fputs(_("\
...
for details.\n\n"));
  ...
  if (opt.warc_filename != 0) {
    if (opt.noclobber) {
      fprintf(stderr, _("WARC output does not work with --no-clobber, "
                        "--no-clobber will be disabled.\n"));
      opt.noclobber = false;
    }
    if (opt.timestamping) {
      fprintf(stderr, _("WARC output does not work with timestamping, "
  ...
  if (opt.always_rest || opt.start_pos >= 0) {
    if (opt.compression == compression_auto) {
      ...
      fprintf(stderr, _("Compression does not work with --continue or"
                        " --start-pos, they will be disabled.\n"));
      opt.always_rest = false;
      opt.start_pos = -1;
    }
...
only if outputting to a regular file.\n"));
  ...
  if ((opt.recursive || opt.page_requisites || nurls > 1 || (opt.input_filename && total_downloaded_bytes != 0)) && total_downloaded_bytes != 0) {
    ...
    logprintf(LOG_NOTQUIET,
              ...
              datetime_str(time(NULL)), wall_time, numurls, human_readable(total_downloaded_bytes, 10, 1), download_time, retr_rate(total_downloaded_bytes, total_download_time));
    xfree(wall_time);
    xfree(download_time);
