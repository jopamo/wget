...
struct options {
  ...
  bool protocol_directories; /* Whether to prepend the scheme to dirs. */
  ...
  const char** excludes; /* Directory patterns to exclude while recursing. */
  const char** includes; /* Directory patterns to include while recursing. */
  ...
  enum keyfile_type { keyfile_pem, keyfile_asn1 } cert_type; /* type of client certificate file */
