...
static int _do_handshake(gnutls_session_t session, int fd, struct st_read_timer* timeout);

#if GNUTLS_VERSION_NUMBER >= 0x030604
static int _do_reauth(gnutls_session_t session, int fd, struct st_read_timer* timeout);
#endif

static int key_type_to_gnutls_type(enum keyfile_type type) {
  switch (type) {
    case keyfile_pem:
...
bool ssl_init(void) {
  ...
  if (ncerts <= 0) {
    ...
    if ((dir = opendir(ca_directory)) == NULL) {
      ...
      while ((dent = readdir(dir)) != NULL) {
        ...
        if ((rc = gnutls_certificate_set_x509_trust_file(credentials, ca_file, GNUTLS_X509_FMT_PEM)) <= 0)
  ...
  if (opt.ca_cert) {
    ...
    if ((rc = gnutls_certificate_set_x509_trust_file(credentials, opt.ca_cert, GNUTLS_X509_FMT_PEM)) <= 0)
...
cert to be of the same type.\n"));
    ...
    gnutls_certificate_set_x509_key_file(credentials, opt.cert_file, opt.private_key, type);
  }
...
static int wgnutls_read(int fd, char* buf, int bufsize, void* arg, double timeout) {
  int ret;
  struct wgnutls_transport_context* ctx = arg;
...
static int wgnutls_write(int fd _GL_UNUSED, char* buf, int bufsize, void* arg) {
  struct wgnutls_transport_context* ctx = arg;
...
static int wgnutls_poll(int fd, double timeout, int wait_for, void* arg) {
  struct wgnutls_transport_context* ctx = arg;

  if ((wait_for & WAIT_FOR_READ) && (ctx->peeklen || gnutls_record_check_pending(ctx->session)))
    return 1;
...
static int wgnutls_peek(int fd, char* buf, int bufsize, void* arg, double timeout) {
  int read = 0;
  struct wgnutls_transport_context* ctx = arg;
  ...
  if (bufsize > offset) { /* let wgnutls_read_timeout() take care about timeout */
                          /*if (timeout && gnutls_record_check_pending (ctx->session) == 0
                              && select_fd (fd, 0.0, WAIT_FOR_READ) <= 0)
                            read = 0;
                          else*/
    read = wgnutls_read_timeout(fd, buf + offset, bufsize - offset, ctx, timeout);
    ctx->last_error = read;
    if (read < 0) {
      if (offset)
        read = 0;
...
static const char* wgnutls_errstr(int fd _GL_UNUSED, void* arg) {
  struct wgnutls_transport_context* ctx = arg;

  if (ctx->last_error > 0 || ((ctx->last_error == GNUTLS_E_AGAIN || ctx->last_error == GNUTLS_E_REHANDSHAKE
#if GNUTLS_VERSION_NUMBER >= 0x030604
...
static void wgnutls_close(int fd, void* arg) {
  struct wgnutls_transport_context* ctx = arg;
...
static struct transport_implementation wgnutls_transport = {wgnutls_read, wgnutls_write, wgnutls_poll, wgnutls_peek, wgnutls_errstr, wgnutls_close};

static int _do_handshake(gnutls_session_t session, int fd, struct st_read_timer* read_timer) {
#ifdef F_GETFL
  int flags = 0;
#endif
  int err;
...
#if GNUTLS_VERSION_NUMBER >= 0x030604
static int _do_reauth(gnutls_session_t session, int fd, struct st_read_timer* read_timer) {
#ifdef F_GETFL
  int flags = 0;
#endif
  int err;
...
static int set_prio_default(gnutls_session_t session) {
  int err = -1;
...
static bool pkp_pin_peer_pubkey(gnutls_x509_crt_t cert, const char* pinnedpubkey) {
  /* Scratch */
...
#define _CHECK_CERT(flag, msg)                             \
  if (status & (flag)) {                                   \
    logprintf(LOG_NOTQUIET, (msg), severity, quote(host)); \
    success = false;                                       \
  }
...
bool ssl_check_certificate(int fd, const char* host) {
  ...
  if (gnutls_certificate_type_get(ctx->session) == GNUTLS_CRT_X509) {
    ...
    if (!gnutls_x509_crt_check_hostname(cert, sni_hostname)) {
      logprintf(LOG_NOTQUIET, _("The certificate's owner does not match hostname %s\n"), quote(sni_hostname));
      success = false;
    }
    xfree(sni_hostname);
