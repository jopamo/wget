...
#define ARCH_SIZE 15
  ...
  struct dsc$descriptor_s arch_descr = {ARCH_SIZE, DSC$K_DTYPE_T, DSC$K_CLASS_S, arch};
...
#define VERS_SIZE 8
  ...
  struct dsc$descriptor_s vers_descr = {VERS_SIZE, DSC$K_DTYPE_T, DSC$K_CLASS_S, vers};
...
int set_ods5_dest(char* path) {
  ...
  */
  ...
  struct dsc$descriptor_s dev_descr = {0, DSC$K_DTYPE_T, DSC$K_CLASS_S, 0};
...
static char* ods2_conform(char* path) {
  char* p;
  char* prd;
  char* prs;
  char* prv;
  ...
  for (p = path; p < prv; p++) {
    ...
    if ((prop & 4)) { /* Dot. */
      if (p < prd) {  /* Before last dot in name. */
        *p = '_';     /* Convert to "_". */
      }
    }
    else if ((prop & (32 + 16)) == 0) { /* ODS2-invalid. */
      if (uchr == '~') {
        *p = '-'; /* Convert to "-". */
      }
      else if (uchr == '@') {
...
int utime(const char* path, const struct utimbuf* times) {
  ...
  struct atrdef ut_atr[3] = {{sizeof(vms_abs_time), ATR$C_CREDATE, UWA vms_abs_time}, {sizeof(vms_abs_time), ATR$C_REVDATE, UWA vms_abs_time}, {0, 0, 0}};
  ...
  struct dsc$descriptor dev_dsc = {0, DSC$K_DTYPE_T, DSC$K_CLASS_S, dev_namx};

  struct dsc$descriptor fib_dsc = {sizeof(ut_fib), DSC$K_DTYPE_T, DSC$K_CLASS_S, (char*)&ut_fib};
      ...
      if ((sts & STS$M_SEVERITY) == STS$K_SUCCESS) {
        ...
        if ((sts & STS$M_SEVERITY) == STS$K_SUCCESS) {
          ...
          if ((sts & STS$M_SEVERITY) == STS$K_SUCCESS) {
            ...
            sts = sys$qiow(0, chan, IO$_MODIFY, &iosb_q, 0, 0, &fib_dsc, 0, 0, 0, ut_atr, 0);
...
struct {
  ...
  int term;
} jpi_itm_lst = {{2, JPI$_RMS_EXTEND_SIZE, &rms_ext, &rms_ext_len}, {1, JPI$_RMS_DFMBC, &rms_mbc, &rms_mbc_len}, {1, JPI$_RMS_DFMBFSDK, &rms_mbf, &rms_mbf_len}, 0};
...
int get_rms_defaults() {
  ...
  if (vms_init_diag() > 0) {
    ...
    if (rms_defaults_known > 0) {
      fprintf(stderr, "               Default: deq = %6d, mbc = %3d, mbf = %3d.\n", rms_ext, rms_mbc, rms_mbf);
    }
  }
  return sts;
}
...
int acc_cb(int* id_arg, struct FAB* fab, struct RAB* rab) {
  ...
  if (rms_defaults_known > 0) {
    ...
    if (vms_init_diag() > 0) {
      fprintf(stderr, "Open callback.  ID = %d, deq = %6d, mbc = %3d, mbf = %3d.\n", *id_arg, fab->fab$w_deq, rab->rab$b_mbc, rab->rab$b_mbf);
    }
  }
...
struct tm* localtime_r(t, tp) const time_t* t;
struct tm* tp;
{
